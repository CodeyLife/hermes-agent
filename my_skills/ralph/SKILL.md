...
0. **Pre-context intake (required before planning/execution loop starts)**:
   - Assemble or load a context snapshot at `.hermes/context/{task-slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`).
...
5. **Visual task gate (when screenshot/reference images are present)**:
   ...
   - Persist verdict to `.hermes/state/{scope}/ralph-progress.json` including numeric + qualitative feedback.
...
1. Run deep-interview in quick mode before creating PRD artifacts:
   ...
   - Persist interview output to `.hermes/interviews/{slug}-{timestamp}.md`
2. Create canonical PRD/progress artifacts:
   - PRD: `.hermes/plans/prd-{slug}.md`
   - Progress ledger: `.hermes/state/{scope}/ralph-progress.json` (session scope when available, else root scope)
...
5. Initialize canonical progress ledger at `.hermes/state/{scope}/ralph-progress.json`
...
Workflow: Detect flag, extract task, create `.hermes/plans/prd-{slug}.md`, create `.hermes/state/{scope}/ralph-progress.json`, begin ralph loop.

### Legacy compatibility
- If `.hermes/prd.json` exists and canonical PRD is absent, migrate one-way into `.hermes/plans/prd-{slug}.md`.
- If `.hermes/progress.txt` exists and canonical progress ledger is absent, import one-way into `.hermes/state/{scope}/ralph-progress.json`.
